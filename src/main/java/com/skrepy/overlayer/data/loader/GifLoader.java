...
public class GifLoader {
    ...
    // 共享占位纹理（由主线程创建）
    private static ResourceLocation placeholderTexture = null;
    private final String path;
    private final Path absolutePath;
    private final int id;
    // 加载完成后只读数据（由主线程访问）
    private volatile List<ResourceLocation> gifTextures;
    ...
    private static synchronized ResourceLocation createPlaceholderTexture(TextureManager textureManager) {
        ...
        placeholder.setPixelRGBA(0, 0, 0x00000000);
        DynamicTexture dynTex = new DynamicTexture(placeholder);
        ResourceLocation loc = ResourceLocation.tryBuild("overlayer", "placeholder/" + UUID.randomUUID());
        if (loc == null) loc = ResourceLocation.withDefaultNamespace("placeholder/" + UUID.randomUUID());
    ...
    public synchronized void loadAsync(TextureManager textureManager) {
        ...
        CompletableFuture.supplyAsync(this::decodeFrames, DECODER_EXECUTOR).thenAcceptAsync(frameData -> {
            ...
            List<ResourceLocation> textures = new ArrayList<>(frameData.frames.size());
            ...
            for (int i = 0; i < frameData.frames.size(); i++) {
                ...
                if (frame != null) {
                    ...
                    location = ResourceLocation.tryBuild("overlayer", "gif/" + UUID.randomUUID());
                    if (location == null) {
                        location = ResourceLocation.withDefaultNamespace("gif/" + UUID.randomUUID());
                    }
